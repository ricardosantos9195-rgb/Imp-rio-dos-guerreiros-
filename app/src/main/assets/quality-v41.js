(() => {
  ...
  const valid=(x,y)=>x>=2&&x<=98&&y>=2&&y<=98&&!blocked(x,y)&&!enemyLand(x,y);
  const moveSenate=(x,y,label)=>{const own=ownSenate();if(!own)return window.imperioToast('O Senado ainda não foi carregado');own.style.left=x+'%';own.style.top=y+'%';state.castleX=x;state.castleY=y;choosing=false;map.classList.remove('teleport-mode');window.imperioRender();info.textContent='Senado teleportado para X '+Math.round(x)+' · Y '+Math.round(y)+' ('+label+'). Proteção de 10 minutos ativa.';window.imperioToast('Teletransporte concluído')};
  ...
  map.addEventListener('pointerup',e=>{if(!choosing||e.target.closest('.map-search,.teleport-tools,.node'))return;const rect=world.getBoundingClientRect(),x=(e.clientX-rect.left)/rect.width*100,y=(e.clientY-rect.top)/rect.height*100;if(!valid(x,y)){info.textContent='Quadrado ocupado, terreno inválido ou território inimigo. Escolhe outro.';window.imperioToast('Não podes teleportar para esse quadrado');return}moveSenate(x,y,'escolhido')},true);
})();
